...
namespace ImGui {
	...
	*/
	...
	IMGUI_API void Image(const sdl::Sprite& sprite, const ImVec2& size, const ImVec4& tint_col,
		...
		//Image(reinterpret_cast<ImTextureID>(sprite.getTexture().getGlTexture()), size);
	}
	...
	IMGUI_API bool ManButton(const char* idStr, int& nbr, int max, const sdl::Sprite& noMan,
		...
		for (int i = 1; i <= nbr || nbr == 0 && i == 1; ++i) {
			...
			if (nbr != 0) {
				x = p.x + size.x * (i -1);
				addImageQuad(drawList, man, {x - delta.x, p.y - delta.y}, scaledSize, color);
			} else {
				addImageQuad(drawList, man, {x - delta.x, p.y - delta.y}, scaledSize, color);
				addImageQuad(drawList, noMan, {x - delta.x, p.y - delta.y}, scaledSize, color);
			}
		}
