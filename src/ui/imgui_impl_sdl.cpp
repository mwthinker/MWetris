...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event& sdlEvent) {
    ...
    case SDL_MOUSEBUTTONDOWN:
        {
            if (sdlEvent.button.button == SDL_BUTTON_LEFT) g_MousePressed[0] = true;
            if (sdlEvent.button.button == SDL_BUTTON_RIGHT) g_MousePressed[1] = true;
            if (sdlEvent.button.button == SDL_BUTTON_MIDDLE) g_MousePressed[2] = true;
            return true;
        }
    case SDL_TEXTINPUT:
        {
    ...
    case SDL_KEYUP:
        {
			...
			tetris::logger()->info("[ImGui] Key: {}", (char)sdlEvent.key.keysym.sym);
            return true;
        }
    }
    return false;
