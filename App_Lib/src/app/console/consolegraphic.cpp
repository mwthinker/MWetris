...
void ConsoleGraphic::restart(IPlayer& player, int x, int y, console::Console* console) {
	console_ = console;
	x_ = x + 1;
	y_ = y + 3;
	connection_.disconnect();
	...
	callback(BoardEvent::RESTARTED, player.getTetrisBoard());
}
...
void ConsoleGraphic::callback(BoardEvent gameEvent, const TetrisBoard& tetrisBoard) {
