...
ConsoleTetris::ConsoleTetris() :
	...
	gameConnection_ = tetrisGame_.addGameEventHandler([this](TetrisGameEvent& gameEvent) {
		handleConnectionEvent(gameEvent);
	});
}

ConsoleTetris::~ConsoleTetris() {
	gameConnection_.disconnect();
}
...
void ConsoleTetris::restartCurrentGame() {
	...
	tetrisGame_.createLocalGame(TETRIS_WIDTH, TETRIS_HEIGHT, devices);
}
